...
class OmniSciSource(DataSource):
    ...
    def _make_connection(self):
        select_cmd = "SELECT {} FROM {} LIMIT 10".format(
            ", ".join(self._projection or ["*"]), self._collection
        )
    ...
    def _get_schema(self):
        ...
        if self._dtypes is None:
            ...
            self._dtypes = pandas.DataFrame.from_records(
                self._connection.fetchmany(1)
            ).dtypes
    ...
    def _get_partition(self, _):
        ...
        return pandas.DataFrame.from_records(
            self._connection.fetchall(), columns=self._projection
        )
